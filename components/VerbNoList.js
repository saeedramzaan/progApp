  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      //    const resAnswer = await axios.post('http://localhost:8000/mapi/listQuizNo');
        ...
        setVerseNo(resAnswer.data.verb_id);
      ...
      {verse_no.map((item, index) => (
        ...
        <View key={index}>
          <Button
            ...
            onPress={() => {
              console.log(item);
               const mergedObject = { firstDigit: item, secondDigit: quiz_tense[0] }; // Example other number, replace with your actual number
               navigation.navigate('VerbTaskList', mergedObject);
