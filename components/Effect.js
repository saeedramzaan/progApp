  ...
  for (let i = 0; i < 4; i++) {
    buttons.push(
      <Button
        key={i}
        title={`${mergeArray[i]}`}
        onPress={() => validateAnswer(i, correctAnswer[i], mergeArray[i], repetition_mode)}
      />
    );
  }
      ...
      {buttons.map(button => (
        <View key={button.key} style={{ flexBasis: '50%', padding: 5 }}>
          ...
          <View style={styles.answerPanel}>
            <TouchableOpacity color={backgroundColor} style={styles.button} onPress={() => handleAnswer(currentPage, correctAnswer[i], selectedAnswer, repetition_mode,search_id)} >
